...
[ -n "$IMG" ] && [ -f "$IMG" ] || { echo "set \$IMG to your hubris gimlet-c-emu image (.zip)"; exit 1; }
...
if [ "$READ_SENSOR" = 1 ]; then
  [ -x "$FAUX" ] || { echo "[!] --read-sensor needs faux-mgs; set \$FAUX to your management-gateway-service build" >&2; exit 1; }
